...
class _DrinkFormState extends State<DrinkForm> {
  ...
  void initState() {
    ...
    _compoundController = TextEditingController(text: widget.drink?.compound ?? '');
    ...
    _priceControllers = List.generate(_volumeOptions.length, (index) {
      final volume = _volumes.firstWhere(
            (el) => el.volume == _volumeOptions[index],
        orElse: () => Volume(volume: _volumeOptions[index], price: 0, volumeID: 0),
      );
      return TextEditingController(text: volume.price.toString());
    });
  }
