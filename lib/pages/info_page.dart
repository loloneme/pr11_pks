...
@override
  ...
  Future<void> _addToCart(CartItem cartItem, int userID)async {
    try {
      await ApiService().addToCart(cartItem, userID);
    } catch (e) {
      print('Ошибка: $e');
    }
  }
  ...
  void _showAddToCartModal() async {
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        ...
        bool isCold = temperatureOptions.contains("Холодный");
        bool isHot = temperatureOptions.contains("Горячий");
        int quantity = 1;
                      ...
                      children: [
                        temperatureOptions.length > 1 ? DropdownButton<String>(
                          ...
                          onChanged: (String? newValue) {
                            setModalState(() {
                              isCold = newValue == "Холодный";
                              isHot = newValue == "Горячий";                            });
                          },
                        ) : Text(isCold ? "Холодный" : "Горячий",
                            style: GoogleFonts.sourceSerif4(
                                textStyle: const TextStyle(
                                  fontSize: 18.0,
                                  color: Color.fromRGBO(44, 32, 17, 1.0),
